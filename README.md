...
管理员登录读取 `ADMIN_AUTH_DB` 绑定的留言箱当前账号，必须已完成初始化改密；不使用商店 `admins` 表，也不导入或恢复旧默认密码。直接打开 `http://127.0.0.1:5173/studio`。本地 Wrangler 的共享绑定仍是本地空库，不会自动连接生产留言箱；隔离测试自行创建模拟账号库。

改密请前往留言箱。商店 Cookie、会话、限流、审计与业务数据仍保存在商店 `DB`。旧商店管理员会话会失效；留言箱改密、删除账号或要求改密后，已有商店会话在下次请求时失效。无需新增数据库迁移。
...
| `/studio` | 独立管理员登录；侧栏包含总览、插件管理、审核任务、操作日志、AI 设置、下载源、账号安全七个面板：提交/同步/重审/下架/删除/显式恢复、插件详情抽屉（快照、附件、任务、审计）、AI 配置与连通性测试、下载源配置与针对具体资产的测试、前往留言箱改密 |
...
| `pnpm admin --help` | 共享账号管理说明（导入/设密已停用） |
...
- 管理员认证：只读留言箱 `ADMIN_AUTH_DB`；当前绑定已配置，线上登录未在本次验证。
