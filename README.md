...
「我的提交」里可以上传新版本或重新检查。新版本审核期间旧的批准版本仍可下载，未批准的版本没有公开下载地址。包含无法核验来源的二进制时，审核停为待补充，作者可以改用 GitHub 投稿补齐源码与构建配置。
...
| 地址 | 内容 |
| --- | --- |
| `/` | 名称/描述搜索（`/` 或 `⌘K` 聚焦）、最近更新/收藏最多/下载最多排序、分页、行内收藏与下载、来源标记 |
...
支持 `ar` 与 `tar.gz` 外封装，内部 `control.tar`/`data.tar` 可为 tar 或 gzip。xz/zstd、PAX/GNU 扩展、嵌套归档、资源超限或核心材料不足都会停在「检查未完成」，不会当作通过。二进制只做静态清单、摘要与源码关联，没有动态沙箱、可复现构建或商业病毒扫描。默认预算与取舍见[工程决策](docs/DECISIONS.md)。
