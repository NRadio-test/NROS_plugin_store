...
let aiCalls=0,scanCalls=0,mode='allow';
...
if(url.hostname==='api.cloudmersive.com'){scanCalls++;if(request.headers.get('apikey')!=='e2e-isolated-scan-key')return new Response(null,{status:401});return Response.json({CleanResult:true,FoundViruses:[]});}
...
if(req.url==='/__test/info'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify({tasks:(await db.prepare('SELECT status,internal_reason,public_reason,attempts FROM tasks').all()).results,calls:fixture.calls.map(c=>c.url),aiCalls,scanCalls,sha256:fixture.asset.digest.slice(7),sha256Second:asset2.digest.slice(7),assetSize:fixture.ipk.length}));return;}
