...
const PLUGIN_COLUMNS = "p.id,p.full_name,p.repository_id,p.description,p.status,p.blocked,p.revision,json_extract(s.data,'$.tag') version,(SELECT COUNT(*) FROM favorites f WHERE f.plugin_id=p.id) favorite_count,p.download_count,p.public_reason,p.created_at,p.updated_at,p.checked_at,p.approved_snapshot_id,p.submitter_id,t.status task_status,t.attempts task_attempts,t.created_at last_task_at";
...
const CANDIDATES = "SELECT p.id pluginId,p.full_name fullName,json_extract(s.data,'$.tag') version,a.id assetId,a.name,a.size,a.disabled,a.data FROM (SELECT id,full_name,approved_snapshot_id,updated_at FROM plugins WHERE status='published' AND blocked=0 AND approved_snapshot_id IS NOT NULL ORDER BY updated_at DESC,id LIMIT 50) p JOIN snapshots s ON s.id=p.approved_snapshot_id LEFT JOIN assets a ON a.snapshot_id=s.id ORDER BY p.updated_at DESC,p.id,a.id";
